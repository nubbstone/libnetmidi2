...
23 commits, 305 tests across nine suites, CI green on Ubuntu and macOS.
...
- **None of this has run against real hardware.** Everything was verified against the
  spec and against our own code. Interop with the Teensy was last confirmed at Phase 1,
  23 commits ago. **Putting it on the bench is the highest-value next step**, well
  above anything else in this document.
...
- **`0x11` Invitation Reply: Pending is not implemented.** It needs UI, so it belongs
  to the app. It currently earns NAK `0x01`, which is the correct answer for a command
  we do not support.

Nine bugs were fixed along the way, several of which no test could previously see:
